...
type TableColumnKey = 'entryId' | 'searchId' | 'price' | 'stock';

export type EntrySearchType = 'JAN' | 'ASIN';
...
const buildLocalStorageKeys = (prefix: string) => ({
  RECORDS: `${prefix}-records`,
  SELECTED_IDS: `${prefix}-selected-ids`,
  RESULTS: `${prefix}-results`,
  RESULT_CSV: `${prefix}-result-csv`,
});

const normalizeColumns = (columns: string[]): string[] =>
  columns.map((column) => column.trim().toLowerCase());

const pickValueFromColumns = (
...
export function EntryPage({ config }: EntryPageProps): JSX.Element {
  const {
    identifierLabel,
    pageTitle,
    pageDescription,
  ...
  const storageKeys = useMemo(
    () => buildLocalStorageKeys(storageKeyPrefix),
    [storageKeyPrefix],
  );
  const entryColumnKeys = useMemo(
    () => normalizeColumns(csvEntryKeys),
    [csvEntryKeys],
  );
  const searchColumnKeys = useMemo(
    () => normalizeColumns(searchField?.csvKeys ?? []),
    [searchField],
  );
  const tableColumns = useMemo(() => {
    const columns: { key: TableColumnKey; label: string }[] = [
      { key: 'entryId', label: identifierLabel },
    ];
    if (searchField) {
      columns.push({ key: 'searchId', label: searchField.label });
    }
    columns.push({ key: 'price', label: 'price' });
    columns.push({ key: 'stock', label: 'stock' });
    return columns;
  }, [identifierLabel, searchField]);
  const apiEndpoint = apiEndpointOverride ?? '/api/amazon-entry';

  const validateRecord = useCallback(
    (record: Omit<ParsedRecord, 'id' | 'errors'>): string[] => {
      const errors: string[] = [];
      const identifierError = identifierValidator(record.entryId);
      if (identifierError) {
        errors.push(identifierError);
      }

      if (searchField?.validator) {
        const searchError = searchField.validator(record.searchId ?? '');
        if (searchError) {
    ...
    [identifierValidator, searchField],
  );

  const buildParsedRecords = useCallback(
    (text: string): ParsedRecord[] => {
      ...
      return rows.map((row, index) => {
        ...
        const entryId = pickValueFromColumns(normalized, entryColumnKeys);
        const candidateSearchId =
          searchColumnKeys.length > 0
            ? pickValueFromColumns(normalized, searchColumnKeys)
            : entryId;
        ...
        const baseRecord = {
          id: `${seed}-${index}`,
          entryId,
          searchId: candidateSearchId || entryId,
          price,
          stock,
        };
        return {
    ...
    [entryColumnKeys, searchColumnKeys, validateRecord],
  );

  const getRecordCellValue = useCallback(
    (record: ParsedRecord, key: TableColumnKey): string => {
      switch (key) {
        case 'entryId':
  ...
  const getRecordCellClass = useCallback((key: TableColumnKey): string => {
    if (key === 'entryId' || key === 'searchId') {
      return 'p-3 font-mono text-xs';
    }
    return 'p-3';
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      const storedRecords = window.localStorage.getItem(storageKeys.RECORDS);
      if (storedRecords) {
    ...
    try {
      const storedSelectedIds = window.localStorage.getItem(storageKeys.SELECTED_IDS);
    ...
    try {
      const storedResults = window.localStorage.getItem(storageKeys.RESULTS);
      if (storedResults) {
    ...
    const storedResultCsv = window.localStorage.getItem(storageKeys.RESULT_CSV);
    if (storedResultCsv) {
      setResultCsv(storedResultCsv);
    }
  }, [storageKeys.RECORDS, storageKeys.RESULTS, storageKeys.RESULT_CSV, storageKeys.SELECTED_IDS]);
  ...
  useEffect(() => {
    ...
    if (records.length === 0) {
      window.localStorage.removeItem(storageKeys.RECORDS);
      window.localStorage.removeItem(storageKeys.SELECTED_IDS);
      return;
    }
    ...
    window.localStorage.setItem(
      storageKeys.SELECTED_IDS,
      JSON.stringify(validIds),
    );
  }, [records, selectedIds, storageKeys.RECORDS, storageKeys.SELECTED_IDS]);
  ...
  useEffect(() => {
    ...
    if (results.length === 0) {
      window.localStorage.removeItem(storageKeys.RESULTS);
      return;
    }
    window.localStorage.setItem(
      storageKeys.RESULTS,
  ...
  useEffect(() => {
    ...
    if (resultCsv) {
      window.localStorage.setItem(storageKeys.RESULT_CSV, resultCsv);
    } else {
      window.localStorage.removeItem(storageKeys.RESULT_CSV);
    }
  }, [resultCsv, storageKeys.RESULT_CSV]);
                    ...
                    <tr key={record.id} className="hover:bg-zinc-50/70">
                      ...
                      {tableColumns.map((column) => (
                        <td
                          ...
                          className={getRecordCellClass(column.key)}
                        >
          ...
          {results.length > 0 ? (
            <div className="mt-6 max-h-[28rem] overflow-x-auto overflow-y-auto rounded-2xl border border-zinc-100">
              <table className="min-w-full divide-y divide-zinc-100 text-left text-sm">
                <thead className="bg-zinc-50 text-xs uppercase tracking-wide text-zinc-500">
                  <tr>
                    <th className="p-3">{identifierLabel}</th>
